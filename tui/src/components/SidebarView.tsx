...
export const SidebarView: React.FC<SidebarViewProps> = ({
  ...
  <Box flexDirection="column">
    <SectionHeader
      title={groupingMode === "feed-categories" ? "Feed Categories" : "Item Categories"}
      terminalWidth={terminalWidth}
    />
    {categories.slice(scrollOffset, scrollOffset + visibleHeight).map((c, i) => {
      ...
      if (c.isHeader) {
        ...
        return (
          <Text key={`h-${realIdx}`} bold color="magenta">
            {`  ${rangeId} │ ${c.title.toUpperCase()}`.padEnd(terminalWidth - 4)}
          </Text>
        );
      }
